...
export default async function PaperPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [paper, document, comments] = await Promise.all([findPaper(id), getDocumentInfo(id), listComments(id)]);
  if (!paper) notFound();
  return <><SiteHeader /><main id="main" className="shell page-shell"><Link href="/papers" className="back-link">← 논문 아카이브</Link>
    ...
    <div className="detail-grid"><div><div className="panel">{paperSections.map(({ key, label }) => <section key={key} className="note-section"><h2>{label}</h2><p className={"note-text " + (!paper[key] ? "muted" : "")}>{paper[key] || "아직 정리한 내용이 없습니다."}</p></section>)}</div><Comments paperId={id} comments={comments} /></div>
