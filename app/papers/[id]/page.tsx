...
export default async function PaperPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><SiteHeader /><main id="main" className="shell page-shell"><Link href="/papers" className="back-link">← {copy.common.archiveTitle}</Link>
    <header className="detail-header"><span className={"badge " + (paper.status === "discussed" ? "badge-done" : "")}>{paperStatuses[paper.status]}</span><h1>{paper.title}</h1>{paper.subtitle && <p className="note-text">{paper.subtitle}</p>}<p className="muted">{paper.authors || copy.paper.noAuthors}</p></header>
    <div className="detail-grid"><div><div className="panel">{paperSections.map(({ key, label }) => <section key={key} className="note-section"><h2>{label}</h2><p className={"note-text " + (!paper[key] ? "muted" : "")}>{paper[key] || copy.paper.noNotes}</p></section>)}</div><Comments paperId={id} comments={comments} /></div>
