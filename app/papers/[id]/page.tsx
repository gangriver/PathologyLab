...
export default async function PaperPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><SiteHeader /><main id="main" className="shell page-shell"><Link href="/papers" className="back-link">← {copy.common.archiveTitle}</Link>
    <header className="detail-header"><span className={"badge " + (paper.status === "discussed" ? "badge-done" : "")}>{paperStatuses[paper.status]}</span><h1>{paper.title}</h1><p className="muted">{paper.authors || copy.paper.noAuthors}</p></header>
